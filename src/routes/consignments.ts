...
export async function consignmentRoutes(app: FastifyInstance) {
  ...
  app.post('/admin/consignments/:id/status', async (request, reply) => {
    ...
    const { id } = idParamsSchema.parse(request.params);
    const payload = adminStatusUpdateSchema.parse(request.body);
    ...
    const updated = await prisma.consignmentRequest.update({
      ...
      data: {
        ...
        assignedDealer: payload.assignedDealer
          ? {
              upsert: {
                create: {
                  dealerId: payload.assignedDealer.dealerId,
                  dealerName: payload.assignedDealer.dealerName,
                  city: payload.assignedDealer.city,
                  district: payload.assignedDealer.district,
                  contactName: payload.assignedDealer.contactName,
                  contactPhone: payload.assignedDealer.contactPhone,
                  statusNote: payload.assignedDealer.statusNote ?? '',
                  status: payload.assignedDealer.status ?? DealerAssignmentStatus.PENDING,
                },
                update: {
                  dealerId: payload.assignedDealer.dealerId,
                  dealerName: payload.assignedDealer.dealerName,
          ...
          : undefined,
        timeline: payload.timelineEntry
          ? {
              create: {
                title: payload.timelineEntry.title,
                description: payload.timelineEntry.description,
                done: payload.timelineEntry.done ?? true,
              },
            }
          : undefined,
      },
